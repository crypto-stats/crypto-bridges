...
export async function getSecurityData(id: string): Promise<ISecurityData | null> {
  const response = await fetch(`https://v1.nocodeapi.com/dmihal/airtable/EcuYTRIwIpUHVGwG?tableName=Bridges&filterByFormula=Bridge%3D"${id}%22`);
  const json = await response.json();
  return json.records.length === 0 ? null : json.records[0].fields;
}
